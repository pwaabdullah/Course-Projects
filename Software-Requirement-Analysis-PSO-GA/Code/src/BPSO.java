...
/**
 * This class representing an asynchronous version of BPSO algorithm
 */
public class BPSO extends Algorithm {
	...
	/**
	 * Stores the best_ solutions founds so far for each particles (local best)
	 */
...
//		}
	...
	private void computeNewPositions() throws JMException{
		...
		for (int i = 0; i < particlesSize_; i++){
			...
			for (int var = 0; var < particle.length; var++){
				...
				Binary variable = (Binary) particle[var];
				//iterate over bits
				for (int bit = 0; bit < variable.getNumberOfBits(); bit++)
				{
					...
					{
						// do nothing
					} else if (a < speed && speed <= (2*(1+a)))
					{
						//xid(new) = pid
						//particle[var].setValue(1);
						//Binary tes = (Binary) bestParticle[var].deepCopy();
						boolean pi = ((Binary) bestParticle[var]).getIth(bit);
						variable.setIth(bit, pi);
						
						
					} else if (0.5*(1+a) < speed && speed <= 1)
					{
						//xid(new) = pgd
						//Binary tes = (Binary) bestGlobal[var].deepCopy();
						boolean pg = ((Binary) bestGlobal[var]).getIth(bit);
						//variable = (Binary) bestGlobal[var].deepCopy();
						variable.setIth(bit, pg);
					}
					
					double disipationProbability = new Random().nextDouble();
					if (disipationProbability < 0.15)
						variable.setIth(bit,new Random().nextDouble()>0.5?true:false);
					
					double mutationProbability = new Random().nextDouble();
					if (mutationProbability < 0.15)
						variable.setIth(bit,!variable.getIth(bit));
				}
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		for (int i = 0; i < particles_.size(); i++){
			...
			if (leaders_.add(particle)){
				Configuration.logger_.info("Found a new solution" + particle.getObjective(0) + " , " + particle.getObjective(1));
	...
	/** 
	 * Gets the leaders of the BPSO algorithm
	 */

function startIdx = symbolTiming(x, chanBW, varargin)
%symbolTiming Perform symbol timing based on L-LTF in time-domain
%   STARTIDX = symbolTiming(X, CHANBW) returns the estimated starting index
%   for the Non-HT Long Training Field (L-LTF), using cross-correlation
%   method.
%   
...
%   'CBW160'.
%   
%   STARTIDX = symbolTiming(..., THRESHOLD) specifies the relative
...
%   LLTFIdx = symbolTiming(rxSig, 'CBW40')
%
%   % If the input starts in L-LTF, the returned index can be 0 or negative
%   LLTFIdx = symbolTiming(rxSig(17:end,:), 'CBW40') % Should be 0
%   LLTFIdx = symbolTiming(rxSig(18:end,:), 'CBW40') % Should be -1 
% 
...
if ~any(strcmp(chanBW,{'CBW20','CBW40','CBW80','CBW160'}))
    error('The chanBW input must be one of ''CBW20'', ''CBW40'', ''CBW80'' and ''CBW160''.')
end
...
FFTLen = 64*real(str2double(chanBW(4:end)))/20;
symLen = FFTLen * 5/4;
bufLen = 4*symLen;
xLen = size(x, 1);
